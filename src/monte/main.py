...
class Monte:
    ...
    # --------------------- FIT ---------------------
    def fit(self, X: pd.DataFrame, purity: pd.Series, sample_weights: Optional[pd.Series] = None) -> "Monte":
        """
        ...
        if sample_weights is not None:
            ...
            self.sample_weights_ = pd.Series(np.ones(X.shape[0]), index=X.index, name="sample_weights")
        ...
        self.residual_variance = pd.Series(sig2, index=X.columns, name="residual_variance")
        ...
        self.moderated_variance = pd.Series(sig2_post, index=X.columns, name="moderated_variance")
    ...
    def predict_purity(self, X: pd.DataFrame) -> pd.Series:
        ...
        b = np.asarray(self.coef_, dtype=float)
    ...
    def _calc_sample_weights(additional_meta: pd.DataFrame) -> pd.Series:
        ...
        weights = np.ones_like(score_agreement, dtype=float) - np.asarray(score_agreement, dtype=float)
